...
private:
	...
	Mesh processMesh(aiMesh *mesh, const aiScene *scene)
	{
		...
		std::vector<Texture> heightMaps = loadMaterialTextures(material, aiTextureType_HEIGHT, Texture::HEIGHT);
	...
	std::vector<Texture> loadMaterialTextures(aiMaterial *mat, aiTextureType type, Texture::TextureType typeName)
	{
		...
		for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
		{
			...
			{   // if texture hasn't been loaded already, load it
				Texture texture(std::string(str.C_Str()), this->directory, typeName);
