...
*	Load the textures 
*/
void Mesh::MeshEntry::loadTextures(const aiMaterial *mat, aiTextureType type, Texture::TextureType texType, Mesh *m)
{
	aiString path;
	...
	if (mat->GetTexture(type, 0, &path,
		...
		if (load) {
			Texture texture(fullPath, "", texType);
...
**/
void Mesh::MeshEntry::render(Shader *shader) {

	for (int j = 0; j < textures.size(); j++) {

		std::string type;
		if (textures[j].getType() == Texture::TextureType::DIFFUSE) {
			type = "ColorTex";
		}
		else if (textures[j].getType() == Texture::TextureType::NORMAL) {
			type = "NormalMapTex";
		}

		glActiveTexture(GL_TEXTURE0 + j);
		glUniform1i(glGetUniformLocation(shader->getId(), type.c_str()), j);
